...
# ============================================================
# FUNCION: ANALISIS COMPLETO PARA VARIABLES NUMERICAS DE CSV
# ============================================================
...
with tab1:
    ...
    with col2:
        if 'resultados_lista' in st.session_state:
            ...
            # Mostrar estadisticas avanzadas
            col_res1, col_res2 = st.columns(2)
            with col_res1:
                st.metric("Count", res['stats']['n'])
                st.metric("Average", f"{res['stats']['media']:.4f}")
                st.metric("Median", f"{res['stats']['mediana']:.4f}")
                moda_val = res['stats']['moda']
                if isinstance(moda_val, np.ndarray):
                    moda_str = ', '.join([str(round(m, 4)) for m in moda_val])
                else:
                    moda_str = f"{moda_val:.4f}" if isinstance(moda_val, (int, float)) else str(moda_val)
                st.metric("Mode", moda_str)
            with col_res2:
                st.metric("Variance", f"{res['stats']['varianza']:.4f}")
                st.metric("Std Dev", f"{res['stats']['desviacion']:.4f}")
                st.metric("Range", f"{res['stats']['rango']:.4f}")
                st.metric("IQR", f"{res['stats']['iqr']:.4f}")
            
            # Mostrar mas estadisticas
            with st.expander("Ver todas las estadisticas avanzadas"):
                col_adv1, col_adv2, col_adv3 = st.columns(3)
                with col_adv1:
                    st.metric("Geometric mean", f"{res['stats']['media_geometrica']:.4f}" if not np.isnan(res['stats']['media_geometrica']) else "N/A")
                    st.metric("Harmonic mean", f"{res['stats']['media_armonica']:.4f}" if not np.isnan(res['stats']['media_armonica']) else "N/A")
                    st.metric("5% Trimmed mean", f"{res['stats']['media_truncada_5']:.4f}" if not np.isnan(res['stats']['media_truncada_5']) else "N/A")
                    st.metric("5% Winsorized mean", f"{res['stats']['media_winsorizada_5']:.4f}" if not np.isnan(res['stats']['media_winsorizada_5']) else "N/A")
                with col_adv2:
                    st.metric("Coeff. of variation", f"{res['stats']['cv']:.4f}%" if not np.isnan(res['stats']['cv']) else "N/A")
                    st.metric("Gini coefficient", f"{res['stats']['gini']:.8f}" if not np.isnan(res['stats']['gini']) else "N/A")
                    st.metric("Standard error", f"{res['stats']['error_estandar']:.6f}" if not np.isnan(res['stats']['error_estandar']) else "N/A")
                    st.metric("MAD", f"{res['stats']['mad']:.4f}")
                with col_adv3:
                    st.metric("Skewness", f"{res['stats']['skewness']:.5f}" if not np.isnan(res['stats']['skewness']) else "N/A")
                    st.metric("Stnd. skewness", f"{res['stats']['skewness_std']:.5f}" if not np.isnan(res['stats']['skewness_std']) else "N/A")
                    st.metric("Kurtosis", f"{res['stats']['kurtosis']:.6f}" if not np.isnan(res['stats']['kurtosis']) else "N/A")
                    st.metric("Stnd. kurtosis", f"{res['stats']['kurtosis_std']:.5f}" if not np.isnan(res['stats']['kurtosis_std']) else "N/A")
            ...
            st.subheader("Visualizaciones")
            fig_box = go.Figure()
            fig_box.add_trace(go.Box(y=res['datos'], name="Datos", boxmean='sd', marker_color='#3498db', boxpoints='outliers'))
            ...
            fig_hist.update_layout(
                title="Histograma y Poligono de Frecuencias",
                height=350,
                barmode='overlay',
                xaxis_title="Clases",
                yaxis_title="Frecuencia",
