...
def identificar_distribucion(datos):
    ...
    if len(datos) < 8:
        return {"distribucion": "Datos insuficientes", "pruebas": {}}
    ...
    if columnas_numericas:
        ...
        if columna_seleccionada:
            ...
            if len(datos) > 0:
                ...
                with col_dist1:
                    st.metric("Distribución más probable", distribucion['distribucion'])
                with col_dist2:
                    st.metric("Asimetría", f"{distribucion['skewness']:.3f}")
                with col_dist3:
                    st.metric("Curtosis", f"{distribucion['kurtosis']:.3f}")
                
                st.write("**Pruebas de normalidad:**")
                for prueba, resultados in distribucion['pruebas'].items():
...
with tab1:
    ...
    with col2:
        if 'resultados_lista' in st.session_state:
            ...
            with col_dist1:
                st.metric("Distribución más probable", res['distribucion']['distribucion'])
            with col_dist2:
                st.metric("Asimetría", f"{res['distribucion']['skewness']:.3f}")
