...
def mostrar_analisis_descriptivo(df):
    ...
    # ============================================================
    # NUEVA SECCIÓN: TABLA DINÁMICA (DOCENTE-ASIGNATURA-NRC)
    # ============================================================
    st.subheader("📊 Análisis de Tabla Dinámica (Filas: Docente-Asignatura-NRC)")

    st.info("""
    **Configuración:**
    - **FILAS**: 3 variables cualitativas (Docente → Asignatura → NRC)
    - **COLUMNAS**: 1 variable cualitativa (ej: Modalidad 1, 2, 3)
    - **VALOR**: 1 variable cuantitativa que se suma
    - **FILTRO**: 1 variable cualitativa para filtrar datos
    """)
    ...
    with col_conf1:
        # Variables de fila (jerarquía: Docente → Asignatura → NRC)
        st.markdown("**📌 Variables de FILA (Jerarquía)**")
        var_fila1 = st.selectbox(
            "Fila Nivel 1 (Ej: Docente):",
            todas_columnas,
            key="pivot_fila1",
            help="Variable principal de agrupación"
        )

        # Filtrar para que fila2 no sea igual a fila1
        opciones_fila2 = [col for col in todas_columnas if col != var_fila1]
        var_fila2 = st.selectbox(
            "Fila Nivel 2 (Ej: Asignatura):",
            opciones_fila2,
            key="pivot_fila2",
            help="Segundo nivel de agrupación"
        )

        # Filtrar para que fila3 no sea igual a fila1 o fila2
        opciones_fila3 = [col for col in todas_columnas if col not in [var_fila1, var_fila2]]
        if opciones_fila3:
            var_fila3 = st.selectbox(
                "Fila Nivel 3 (Ej: NRC):",
                opciones_fila3,
                key="pivot_fila3",
                help="Tercer nivel de agrupación (opcional)"
            )
        else:
            var_fila3 = None
            st.warning("No hay más columnas disponibles para el tercer nivel.")

    with col_conf2:
        st.markdown("**📌 Configuración de Columnas, Valores y Filtro**")

        # Variable de columna (cualitativa con 3 modalidades)
        var_columna = st.selectbox(
            "Variable de COLUMNA (cualitativa):",
            columnas_categoricas,
            key="pivot_columna",
            help="Variable con las modalidades (ej: 1, 2, 3)"
        )

        # Variable de valor (cuantitativa)
        if columnas_numericas:
            var_valor = st.selectbox(
                "Variable de VALOR (cuantitativa):",
                columnas_numericas,
                key="pivot_valor",
                help="Variable numérica que se sumará"
            )
        else:
            var_valor = None
        ...
        var_filtro = st.selectbox(
            ...
            key="pivot_filtro",
            help="Variable para filtrar los datos (opcional)"
        )

    # Si hay variables seleccionadas y valores numéricos
    if var_fila1 and var_fila2 and var_columna and var_valor:
        # Crear un lugar para el filtro si se seleccionó
            ...
            with st.spinner("Generando tabla dinámica..."):
                try:
                    ...
                    if len(df_filtrado) == 0:
                        ...
                        # Crear la tabla dinámica con jerarquía de filas
                        # Primero, asegurar que las columnas de fila sean categóricas
                        for col in [var_fila1, var_fila2]:
                        ...
                        if var_fila3:
                            # Con 3 niveles de fila
                            pivot_table = pd.pivot_table(
                                df_filtrado,
                                values=var_valor,
                                index=[var_fila1, var_fila2, var_fila3],
                                columns=var_columna,
                                aggfunc='sum',
                                fill_value=0
                            )
                        else:
                            # Con 2 niveles de fila
                            pivot_table = pd.pivot_table(
                                df_filtrado,
                                values=var_valor,
                                index=[var_fila1, var_fila2],
                                columns=var_columna,
                                aggfunc='sum',
                                fill_value=0
                            )
                        ...
                        with col_info2:
                            st.metric("Suma total", f"{pivot_table['Total General'].sum():,.0f}")
                        with col_info3:
                        ...
                        # ============================================================
                        # DIAGRAMAS DE BARRAS POR NIVEL JERÁRQUICO CON FRECUENCIAS
                        # ============================================================
                        st.subheader("📊 Diagramas de Barras con Frecuencias por Nivel")

                        # Crear tabs para los diferentes niveles
                        tabs_barras = st.tabs([
                            "📊 Por Docente",
                            "📊 Por Asignatura",
                            "📊 Por NRC",
                        ...
                        # Convertir columnas a string para asegurar consistencia
                        for col in [var_fila1, var_fila2]:
                            df_agrupado[col] = df_agrupado[col].astype(str)
                        if var_fila3:
                            df_agrupado[var_fila3] = df_agrupado[var_fila3].astype(str)

                        with tabs_barras[0]:
                            """Barras por Docente (Nivel 1) con frecuencias"""
                            st.markdown(f"**📌 Agrupado por: {var_fila1}**")

                            # Agrupar por docente
                            df_docente = df_agrupado.groupby(var_fila1).agg({
                                var_valor: 'sum',
                                var_columna: lambda x: len(x.unique())
                            }).reset_index()

                            # Ordenar de mayor a menor
                            df_docente = df_docente.sort_values(var_valor, ascending=False)

                            # Calcular estadísticas
                            total_general_doc = df_docente[var_valor].sum()
                            df_docente['Porcentaje'] = (df_docente[var_valor] / total_general_doc * 100).round(1)

                            # Crear gráfico de barras con etiquetas
                            fig_docente = go.Figure()

                            fig_docente.add_trace(go.Bar(
                                x=df_docente[var_fila1],
                                y=df_docente[var_valor],
                                text=df_docente.apply(lambda row: f"{row[var_valor]:,.0f}\n({row['Porcentaje']:.1f}%)", axis=1),
                                textposition='outside',
                                marker_color='#3498db',
                                opacity=0.85,
                                name='Total',
                            ...
                            fig_docente.update_layout(
                                title=f"Total de {var_valor} por {var_fila1}",
                                xaxis_title=var_fila1,
                                yaxis_title=f"Total {var_valor}",
                                height=450,
                                showlegend=False,
                                xaxis={'tickangle': -45}
                            )
                            ...
                            # Mostrar tabla de resumen por docente
                            st.markdown("**📋 Resumen por Docente:**")
                            df_docente_display = df_docente.copy()
                            df_docente_display.columns = [var_fila1, f'Total {var_valor}', 'Modalidades', 'Porcentaje']
                            df_docente_display['Porcentaje'] = df_docente_display['Porcentaje'].apply(lambda x: f'{x:.1f}%')
                            st.dataframe(df_docente_display, use_container_width=True)

                        with tabs_barras[1]:
                            """Barras por Asignatura (Nivel 2) con frecuencias"""
                            st.markdown(f"**📌 Agrupado por: {var_fila2}**")

                            # Agrupar por asignatura
                            df_asignatura = df_agrupado.groupby(var_fila2).agg({
                                var_valor: 'sum',
                                var_columna: lambda x: len(x.unique())
                            }).reset_index()

                            # Ordenar de mayor a menor
                            df_asignatura = df_asignatura.sort_values(var_valor, ascending=False)

                            # Calcular estadísticas
                            total_general_asig = df_asignatura[var_valor].sum()
                            df_asignatura['Porcentaje'] = (df_asignatura[var_valor] / total_general_asig * 100).round(1)

                            # Crear gráfico de barras con etiquetas
                            fig_asignatura = go.Figure()

                            fig_asignatura.add_trace(go.Bar(
                                x=df_asignatura[var_fila2],
                                y=df_asignatura[var_valor],
                                text=df_asignatura.apply(lambda row: f"{row[var_valor]:,.0f}\n({row['Porcentaje']:.1f}%)", axis=1),
                                textposition='outside',
                                marker_color='#2ecc71',
                                opacity=0.85,
                                name='Total',
                            ...
                            fig_asignatura.update_layout(
                                title=f"Total de {var_valor} por {var_fila2}",
                                xaxis_title=var_fila2,
                                yaxis_title=f"Total {var_valor}",
                                height=450,
                                showlegend=False,
                                xaxis={'tickangle': -45}
                            ...
                            # Mostrar tabla de resumen por asignatura
                            st.markdown("**📋 Resumen por Asignatura:**")
                            df_asignatura_display = df_asignatura.copy()
                            df_asignatura_display.columns = [var_fila2, f'Total {var_valor}', 'Modalidades', 'Porcentaje']
                            df_asignatura_display['Porcentaje'] = df_asignatura_display['Porcentaje'].apply(lambda x: f'{x:.1f}%')
                            st.dataframe(df_asignatura_display, use_container_width=True)

                        with tabs_barras[2]:
                            """Barras por NRC (Nivel 3) con frecuencias"""
                            if var_fila3:
                                st.markdown(f"**📌 Agrupado por: {var_fila3}**")

                                # Agrupar por NRC
                                df_nrc = df_agrupado.groupby(var_fila3).agg({
                                    var_valor: 'sum',
                                    var_columna: lambda x: len(x.unique())
                                }).reset_index()

                                # Ordenar de mayor a menor
                                df_nrc = df_nrc.sort_values(var_valor, ascending=False)

                                # Calcular estadísticas
                                total_general_nrc = df_nrc[var_valor].sum()
                                df_nrc['Porcentaje'] = (df_nrc[var_valor] / total_general_nrc * 100).round(1)

                                # Limitar a top 30 para mejor visualización
                                top_nrc = 30
                                if len(df_nrc) > top_nrc:
                                    st.info(f"Mostrando los {top_nrc} NRC con mayor total de {len(df_nrc)} NRC totales")
                                    df_nrc_display = df_nrc.head(top_nrc)
                                else:
                                    df_nrc_display = df_nrc

                                # Crear gráfico de barras
                                fig_nrc = go.Figure()

                                fig_nrc.add_trace(go.Bar(
                                    x=df_nrc_display[var_fila3].astype(str),
                                    y=df_nrc_display[var_valor],
                                    text=df_nrc_display.apply(lambda row: f"{row[var_valor]:,.0f}\n({row['Porcentaje']:.1f}%)", axis=1),
                                    textposition='outside',
                                    marker_color='#e74c3c',
                                    opacity=0.85,
                                    name='Total',
                                ...
                                fig_nrc.update_layout(
                                    title=f"Total de {var_valor} por {var_fila3} (Top {top_nrc})",
                                    xaxis_title=var_fila3,
                                    yaxis_title=f"Total {var_valor}",
                                    height=500,
                                    showlegend=False,
                                    xaxis={'tickangle': -90}
                                )
                                st.plotly_chart(fig_nrc, use_container_width=True)

                                # Mostrar tabla de resumen por NRC
                                st.markdown("**📋 Resumen por NRC (Top 20):**")
                                df_nrc_display_table = df_nrc.head(20).copy()
                                df_nrc_display_table.columns = [var_fila3, f'Total {var_valor}', 'Modalidades', 'Porcentaje']
                                df_nrc_display_table['Porcentaje'] = df_nrc_display_table['Porcentaje'].apply(lambda x: f'{x:.1f}%')
                                st.dataframe(df_nrc_display_table, use_container_width=True)
                            else:
                                st.info("ℹ️ No se seleccionó una variable para NRC (Nivel 3)")

                        with tabs_barras[3]:
                            """Barras por Modalidad (Columnas 1, 2, 3) con frecuencias"""
                            st.markdown(f"**📌 Distribución por {var_columna}**")

                            # Agrupar por modalidad
                            df_modalidad = df_agrupado.groupby(var_columna).agg({
                                var_valor: 'sum'
                            }).reset_index()

                            # Asegurar que todas las modalidades estén presentes
                            modalidades_esperadas = sorted(df_agrupado[var_columna].dropna().unique())

                            # Calcular total y porcentajes
                            total_general_mod = df_modalidad[var_valor].sum()
                            df_modalidad['Porcentaje'] = (df_modalidad[var_valor] / total_general_mod * 100).round(1)

                            # Crear gráfico de barras
                            fig_modalidad = go.Figure()

                            # Colores para cada modalidad
                            colores_modalidad = ['#3498db', '#2ecc71', '#e74c3c', '#f39c12', '#9b59b6', '#1abc9c']

                            for i, mod in enumerate(modalidades_esperadas):
                                valor = df_modalidad[df_modalidad[var_columna] == mod][var_valor].sum()
                                porcentaje = df_modalidad[df_modalidad[var_columna] == mod]['Porcentaje'].values[0]
                                fig_modalidad.add_trace(go.Bar(
                                    x=[str(mod)],
                                    y=[valor],
                                    text=[f'{valor:,.0f}\n({porcentaje:.1f}%)'],
                                    textposition='outside',
                                    marker_color=colores_modalidad[i % len(colores_modalidad)],
                                    opacity=0.85,
                                    name=f'{var_columna} = {mod}',
                                    hovertemplate=f'<b>{var_columna} = {mod}</b><br>Total: {{y:,.0f}}<br>Porcentaje: {porcentaje:.1f}%<extra></extra>'
                                ))

                            fig_modalidad.update_layout(
                                title=f"Total de {var_valor} por {var_columna}",
                            ...
                            # Mostrar tabla de resumen por modalidad
                            st.markdown("**📋 Resumen por Modalidad:**")
                            df_modalidad_display = df_modalidad.copy()
                            df_modalidad_display.columns = [var_columna, f'Total {var_valor}', 'Porcentaje']
                            df_modalidad_display['Porcentaje'] = df_modalidad_display['Porcentaje'].apply(lambda x: f'{x:.1f}%')
                            st.dataframe(df_modalidad_display, use_container_width=True)

                            # Gráfico circular de distribución
                            fig_pie_modalidad = px.pie(
                                df_modalidad_display,
                                values=f'Total {var_valor}',
                                names=var_columna,
                                title=f"Distribución Porcentual de {var_valor} por {var_columna}",
                                color_discrete_sequence=colores_modalidad[:len(modalidades_esperadas)]
                            )
                            fig_pie_modalidad.update_traces(textinfo='label+percent+value')
                            fig_pie_modalidad.update_layout(height=400)
                            st.plotly_chart(fig_pie_modalidad, use_container_width=True)

                        with tabs_barras[4]:
                            """Comparativa Completa: Docente vs Asignatura vs Modalidad"""
                            st.markdown("**📌 Comparativa Detallada por Docente y Modalidad**")

                            # Verificar que hay suficientes datos
                            if len(df_agrupado[var_fila1].unique()) > 1 and len(df_agrupado[var_columna].unique()) > 1:
                                try:
                                    # Crear tabla de calor por docente y modalidad
                                    pivot_docente_modalidad = pd.pivot_table(
                                        df_agrupado,
                                        values=var_valor,
                                        index=var_fila1,
                                        columns=var_columna,
                                    ...
                                    # Agregar total
                                    pivot_docente_modalidad['Total'] = pivot_docente_modalidad.sum(axis=1)
                                    pivot_docente_modalidad = pivot_docente_modalidad.sort_values('Total', ascending=False)

                                    # Calcular porcentajes por fila
                                    pivot_pct = pivot_docente_modalidad.copy()
                                    for col in pivot_pct.columns:
                                        if col != 'Total':
                                            pivot_pct[col] = (pivot_pct[col] / pivot_pct['Total'] * 100).round(1)

                                    # Gráfico de barras apiladas por docente
                                    pivot_long = pivot_docente_modalidad.reset_index().melt(
                                        id_vars=[var_fila1],
                                        var_name=var_columna,
                                        value_name=var_valor
                                    )

                                    # Limitar a top 20 docentes para visualización
                                    top_docentes = pivot_docente_modalidad.head(20).index.tolist()
                                    pivot_long_top = pivot_long[pivot_long[var_fila1].isin(top_docentes)]
                                    ...
                                    fig_comparativa = px.bar(
                                        ...
                                        barmode='stack',
                                        text=var_valor,
                                        title=f"Distribución de {var_valor} por {var_fila1} y {var_columna} (Top 20)",
                                        color_discrete_sequence=px.colors.qualitative.Set2,
                                        hover_data={var_valor: ':.0f'}
                                    )
                                    ...
                                    fig_comparativa.update_layout(
                                        ...
                                        yaxis_title=f"Total {var_valor}",
                                        xaxis={'tickangle': -45},
                                        legend_title=var_columna
                                    )
                                    st.plotly_chart(fig_comparativa, use_container_width=True)

                                    # Mostrar tabla detallada
                                    st.markdown("**📋 Tabla Detallada por Docente y Modalidad:**")

                                    # Crear display con valores y porcentajes
                                    pivot_display = pivot_docente_modalidad.copy()
                                    ...
                                    st.markdown("**📊 Porcentajes por Fila (Distribución por Docente):**")
                                    pivot_pct_display = pivot_pct.copy()
                                    for col in pivot_pct_display.columns:
                                        if col != 'Total':
                                            pivot_pct_display[col] = pivot_pct_display[col].apply(lambda x: f"{x:.1f}%")
                                    st.dataframe(pivot_pct_display, use_container_width=True)

                                except Exception as e:
                                    st.warning(f"No se pudo generar la comparativa completa: {str(e)}")
                            else:
                                st.info("Se necesitan al menos 2 docentes y 2 modalidades para la comparativa completa.")

                except Exception as e:
                    st.error(f"Error al generar la tabla dinámica: {str(e)}")
                    st.exception(e)
    else:
        st.warning("Selecciona todas las variables necesarias para generar la tabla dinámica.")
