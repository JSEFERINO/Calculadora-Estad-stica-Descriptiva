...
def procesar_contingencia(texto, filas, columnas):
    """
    Procesa tabla de contingencia en formato CSV:
    Fila, Columna, Valor; Fila, Columna, Valor; ...
    """
    if not texto:
        return None
...
def mostrar_analisis_descriptivo(df):
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
        ...
        "📊 Tabla Dinámica",
        "📊 Tablas de Contingencia",  # NUEVA PESTAÑA
    ...
    with tab4:
        ...
        **Configuración:**
        ...
        - **ESTADÍSTICO**: Promedio, Conteo, Máximo, Mínimo, Mediana, Suma o Conteo en Rango Personalizado
        ...
        with col_conf2:
            ...
            if columnas_numericas:
                ...
                estadistico = st.selectbox(
                    "Estadístico a calcular:",
                    ["Suma", "Promedio", "Conteo", "Máximo", "Mínimo", "Mediana", "Conteo en Rango (Personalizado)"],
                    key="pivot_estadistico"
                )

                # Lógica para mostrar los inputs del rango personalizado
                if estadistico == "Conteo en Rango (Personalizado)":
                    st.markdown("**🔢 Definir Rango de Conteo**")
                    col_rango1, col_rango2 = st.columns(2)
                    with col_rango1:
                        rango_min = st.number_input("Límite Inferior (>=):", value=0.0, step=0.1, key="pivot_rango_min")
                    with col_rango2:
                        rango_max = st.number_input("Límite Superior (<=):", value=5.0, step=0.1, key="pivot_rango_max")
                # ----------------------------------
            else:
                var_valor = None
                estadistico = "Suma"
        ...
        if st.button("🔍 Generar Tabla Dinámica", key="btn_pivot_table"):
            ...
            with st.spinner("Generando tabla dinámica..."):
                try:
                    ...
                    # --- LÓGICA MODIFICADA PARA RANGO PERSONALIZADO ---
                    if estadistico == "Conteo en Rango (Personalizado)":
                        # Usamos una función lambda que cuenta cuántos valores caen en el rango [rango_min, rango_max]
                        def contar_en_rango(serie):
                            return serie.between(rango_min, rango_max).sum()
                        agg_func = contar_en_rango
                    else:
                    ...
                    for idx, var in enumerate([v for v in index if v != "(Ninguna)"]):
                        with tabs_barras[idx]:
                            # Aquí también usamos la función de agregación personalizada
    ...
    # ============================================================
    # PESTAÑA 5: TABLAS DE CONTINGENCIA (NUEVA)
    # ============================================================
    with tab5:
        st.subheader("📊 Análisis de Tablas de Contingencia")
        
        columnas_categoricas = df.select_dtypes(include=['object', 'category']).columns.tolist()
        
        if len(columnas_categoricas) < 2:
            st.warning("Se necesitan al menos 2 variables categóricas para realizar este análisis.")
        else:
            col_conf1, col_conf2 = st.columns(2)
            with col_conf1:
                var_fila = st.selectbox("Variable para Filas:", columnas_categoricas, key="cont_var_fila")
            with col_conf2:
                opciones_col = [c for c in columnas_categoricas if c != var_fila]
                var_col = st.selectbox("Variable para Columnas:", opciones_col, key="cont_var_col")
            
            if st.button("🔍 Analizar Tabla de Contingencia", key="btn_analizar_contingencia"):
                with st.spinner("Generando análisis..."):
                    # Crear tabla de contingencia
                    tabla = pd.crosstab(df[var_fila], df[var_col])
                    
                    if tabla.size == 0:
                        st.warning("La tabla de contingencia está vacía. Verifica los datos.")
                    else:
                        # Calcular frecuencias esperadas y chi-cuadrado
                        ...
                        with col_res4:
                            st.metric("V de Cramer", f"{cramer_v:.4f}")
                        ...
                        with st.expander("🔬 Ver Frecuencias Esperadas"):
                            df_esperadas = pd.DataFrame(expected, index=tabla.index, columns=tabla.columns)
                        ...
                        df_long = tabla.reset_index().melt(id_vars=var_fila, var_name=var_col, value_name='Frecuencia')
                        
                        # Crear pestañas para los gráficos
                        graf_tab1, graf_tab2, graf_tab3, graf_tab4, graf_tab5 = st.tabs([
                            "Barras Agrupadas", 
                            "Barras Apiladas", 
                            "Porcentual", 
                            "Mapa de Calor",
                            "Gráfico Lineal"  # NUEVA PESTAÑA
                        ])
                        ...
                        with graf_tab2:
                            # CORRECCIÓN: Usamos la tabla original para apilar correctamente
                            fig_stacked = px.bar(
                                tabla, 
                                x=tabla.index, 
                                y=tabla.columns, 
                            ...
                            fig_stacked.update_traces(texttemplate='%{y}', textposition='inside')
                            fig_stacked.update_layout(height=400, xaxis_title=var_fila, yaxis_title="Frecuencia")
                            st.plotly_chart(fig_stacked, use_container_width=True)
                        ...
                        with graf_tab3:
                            ...
                            for fila in tabla.index:
                                mask = df_pct[var_fila] == fila
                                total = df_pct[mask]['Frecuencia'].sum()
                                if total > 0:
                                    df_pct.loc[mask, 'Porcentaje'] = df_pct.loc[mask, 'Frecuencia'] / total * 100
                                else:
                                    df_pct.loc[mask, 'Porcentaje'] = 0
                            ...
                            fig_pct = px.bar(
                                ...
                                color=var_col, 
                                barmode='stack', 
                                text=[f"{v:.1f}%" for v in df_pct['Porcentaje']], 
                        ...
                        with graf_tab5:
                            ...
                            fig_line = px.line(
                                ...
                                color=var_col,
                                markers=True,
                                title='Tendencias por Categoría (Gráfico Lineal)',
                                labels={var_fila: var_fila, 'Frecuencia': 'Número de Casos', var_col: var_col}
                            )
                            fig_line.update_traces(line=dict(width=3), marker=dict(size=10))
                            fig_line.update_layout(
                                height=450,
                                xaxis_title=var_fila,
                                yaxis_title="Frecuencia",
                                legend_title=var_col,
                                hovermode='x unified'
                            )
                            st.plotly_chart(fig_line, use_container_width=True)
                            st.caption("Este gráfico muestra cómo varía la frecuencia de cada categoría a través de las diferentes filas.")

    # ============================================================
    # PESTAÑA 6: ESTADÍSTICAS AVANZADAS
    # ============================================================
    with tab6:
...
with tab6:
    ...
    with col1:
        ...
        st.caption("Formato: fila, columna, valor; fila, columna, valor; ...")
        st.caption("Ejemplo: G1,Ansiedad,24; G1,Depresión,28; G1,Estrés,30; ...")
        datos_contingencia = st.text_area("Datos de la tabla", placeholder='Ejemplo: G1,Ansiedad,24; G1,Depresión,28; G1,Estrés,30; G2,Ansiedad,30; ...', height=150)
        nombres_filas = st.text_input("Nombres de filas (separados por comas):", value="G1, G2, G3, G4")
        nombres_columnas = st.text_input("Nombres de columnas (separados por comas):", value="Ansiedad, Depresión, Estrés")
        if st.button("Calcular", key="calcular_contingencia"):
            ...
            if resultado is None:
                st.error("Error: Verifica el formato de la tabla de contingencia (Fila, Columna, Valor;).")
            else:
    ...
    with col2:
        if 'resultados_contingencia' in st.session_state:
            ...
            # PESTAÑAS ACTUALIZADAS CON GRÁFICO LINEAL
            graf_tab1, graf_tab2, graf_tab3, graf_tab4, graf_tab5 = st.tabs([
                "Barras Agrupadas", 
                "Barras Apiladas", 
                "Porcentual", 
            ...
            with graf_tab1:
                ...
                st.plotly_chart(fig_grouped, use_container_width=True)
            with graf_tab2:
                # CORRECCIÓN DEFINITIVA: Usamos la tabla original para apilar correctamente
                fig_stacked = px.bar(
                    pd.DataFrame(res['matriz'], index=res['filas'], columns=res['columnas']), 
                    x=res['filas'], 
                    y=res['columnas'], 
                    barmode='stack', 
                    title='Barras Apiladas', 
                ...
                fig_stacked.update_traces(texttemplate='%{y}', textposition='inside')
                fig_stacked.update_layout(height=400, xaxis_title="Fila", yaxis_title="Frecuencia")
                st.plotly_chart(fig_stacked, use_container_width=True)
            with graf_tab3:
                df_pct = df_long.copy()
            ...
            with graf_tab5:
                ...
                fig_line = px.line(
                    ...
                    markers=True,
                    title='Tendencias por Categoría (Gráfico Lineal)',
                    labels={'Fila': 'Categoría (Filas)', 'Frecuencia': 'Número de Casos', 'Columna': 'Categoría (Columnas)'}
                )
                fig_line.update_traces(line=dict(width=3), marker=dict(size=10))
                fig_line.update_layout(
                    height=450,
                    xaxis_title="Categoría (Filas)",
                    yaxis_title="Frecuencia",
                    legend_title="Categoría (Columnas)",
                    hovermode='x unified'
                )
                st.plotly_chart(fig_line, use_container_width=True)
                st.caption("Este gráfico muestra cómo varía la frecuencia de cada categoría de columna a través de las diferentes filas.")
