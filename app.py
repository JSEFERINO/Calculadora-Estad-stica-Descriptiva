...
# ============================================================
# FUNCIÓN PARA ANÁLISIS DESCRIPTIVO DE DATAFRAMES CARGADOS
# ============================================================
                ...
                with st.spinner("Analizando relación entre variables..."):
                    ...
                    # CASO 2: CUANTITATIVA-CUANTITATIVA
                        ...
                        if 'pendiente' in resultados['estadisticos']:
                            ...
                            y_pred = resultados['estadisticos']['intercepto'] + resultados['estadisticos']['pendiente'] * datos[var1]
                            residuos = datos[var2] - y_pred
                            ...
                            q_theoretical = stats.norm.ppf(quantiles, np.mean(residuos), np.std(residuos))
                            q_empirical = np.sort(residuos)
                    ...
                    # CASO 3: CUALITATIVA-CUANTITATIVA
                        ...
                        st.subheader("🎻 Violin Plots por Categoría")
                        fig_violin = px.violin(
                            ...
                            color=var_cat,
                            box=True,
                            title="Violin Plots por Categoría"
                        )
                        fig_violin.update_layout(height=400)
                        st.plotly_chart(fig_violin, use_container_width=True)

                        st.subheader("📊 Medias por Categoría")
                        medias = df.groupby(var_cat)[var_cuant].mean().reset_index()
                        errores = df.groupby(var_cat)[var_cuant].std().reset_index()
                        ...
                        fig_means.add_trace(go.Bar(
                            ...
                            error_y=dict(
                                type='data',
                                array=errores[var_cuant],
                                visible=True
                            )
                        ))
                        fig_means.update_layout(
                            title="Medias con Error Estándar",
                            xaxis_title=var_cat,
                            yaxis_title=f"Media de {var_cuant}",
                            height=400
                        )
...
# ============================================================
# PESTAÑAS COMPLETAS (CON ANÁLISIS AVANZADO RESTAURADO Y VISUALIZACIÓN)
# ============================================================
...
# PESTAÑA 1: DATOS EN LISTA (AVANZADO)
with tab1:
    col1, col2 = st.columns([1, 2])
    with col1:
        st.subheader("📊 Ingreso de Datos")
        st.caption("Ingresa los datos separados por comas o espacios")
        datos_texto = st.text_area("Datos:", placeholder="Ejemplo: 12, 15, 18, 20, 22", height=150)
        if st.button("🔍 Calcular", key="calcular_lista"):
            datos = procesar_datos_lista(datos_texto)
            if datos is None: st.error("❌ Error: Ingresa al menos 2 números válidos.")
            else:
                ...
                outliers_info = detectar_outliers(datos); distribucion = identificar_distribucion(datos)
                ...
                for i in range(len(hist_data[0])):
                    if i == 0: clase = f"[{hist_data[1][i]:.2f}, {hist_data[1][i+1]:.2f}]"
                    else: clase = f"({hist_data[1][i]:.2f}, {hist_data[1][i+1]:.2f}]"
                    clases.append(clase)
                ...
                st.session_state['resultados_lista'] = {
                    'datos': datos, 'n': n, 'media': media, 'mediana': mediana, 'moda': moda, 'moda_str': moda_str,
                    'varianza': varianza, 'desviacion': desviacion, 'rango': rango, 'q1': q1, 'q3': q3, 'iqr': iqr,
                    'outliers_info': outliers_info, 'distribucion': distribucion, 'clases': clases,
                    'frecuencias': hist_data[0].tolist(), 'marcas_clase': marcas_clase, 'freq_acum': freq_acum.tolist()
                }
    ...
    with col2:
        if 'resultados_lista' in st.session_state:
            ...
            with col_res1:
                ...
                st.metric("Mediana", f"{res['mediana']:.4f}"); st.metric("Moda", res['moda_str'])
            with col_res2:
                st.metric("Varianza", f"{res['varianza']:.4f}"); st.metric("Desviación Estándar", f"{res['desviacion']:.4f}")
                st.metric("Rango", f"{res['rango']:.4f}"); st.metric("IQR", f"{res['iqr']:.4f}")
            st.subheader("📈 Identificación de Distribución")
            col_dist1, col_dist2 = st.columns(2)
            with col_dist1: st.metric("Distribución más probable", res['distribucion'].get('distribucion', 'No determinada'))
            with col_dist2: st.metric("Asimetría", f"{res['distribucion'].get('skewness', 0):.3f}")
            ...
            st.subheader("📊 Tabla de Frecuencias")
            df_frec = pd.DataFrame({'Clase': res['clases'], 'Frecuencia': res['frecuencias'], 'Marca': [round(m, 2) for m in res['marcas_clase']], 'F. Acumulada': res['freq_acum']})
            ...
            fig_hist = go.Figure(); fig_hist.add_trace(go.Histogram(x=res['datos'], nbinsx=len(res['clases']), marker_color='#3498db', opacity=0.7, name="Histograma"))
            fig_hist.add_trace(go.Scatter(x=res['marcas_clase'], y=res['frecuencias'], mode='lines+markers', name='Polígono', line=dict(color='#e74c3c', width=2), marker=dict(size=6)))
            fig_hist.update_layout(title="Histograma y Polígono de Frecuencias", height=350); st.plotly_chart(fig_hist, use_container_width=True)
            fig_ojiva = go.Figure(); fig_ojiva.add_trace(go.Scatter(x=res['marcas_clase'], y=res['freq_acum'], mode='lines+markers', name='Ojiva', line=dict(color='#2ecc71', width=2), marker=dict(size=6)))
            fig_ojiva.update_layout(title="Ojiva (Frecuencias Acumuladas)", height=350); st.plotly_chart(fig_ojiva, use_container_width=True)
...
with tab2:
    ...
    with col1:
        ...
        datos_agrupados_texto = st.text_area("Datos agrupados:", placeholder="Ejemplo: 10-20, 5; 20-30, 8; 30-40, 12", height=150)
        if st.button("🔍 Calcular", key="calcular_agrupados"):
            resultado = procesar_datos_agrupados(datos_agrupados_texto)
            if resultado is None: st.error("❌ Error: Ingresa al menos 2 clases con frecuencias válidas.")
            else:
                clases = resultado['clases']; frecuencias = resultado['frecuencias']; marcas_clase = resultado['marcas']; limites_inferiores = [float(c.split('-')[0]) for c in clases]; limites_superiores = [float(c.split('-')[1]) for c in clases]
                ...
                f_clase = frecuencias[clase_mediana]; L_mediana = limites_inferiores[clase_mediana]; A_mediana = limites_superiores[clase_mediana] - limites_inferiores[clase_mediana]
                mediana = L_mediana + ((n_mitad - f_anterior) / f_clase) * A_mediana
                clase_modal = np.argmax(frecuencias); f_modal = frecuencias[clase_modal]; f_pre = frecuencias[clase_modal - 1] if clase_modal > 0 else 0; f_post = frecuencias[clase_modal + 1] if clase_modal < len(frecuencias) - 1 else 0
                ...
                def calcular_cuantil(p):
                    ...
                    f_clase = frecuencias[clase_cuantil]; L_cuantil = limites_inferiores[clase_cuantil]; A_cuantil = limites_superiores[clase_cuantil] - limites_inferiores[clase_cuantil]
                ...
                st.session_state['resultados_agrupados'] = {
                    'clases': clases, 'frecuencias': frecuencias, 'marcas_clase': marcas_clase, 'n': n,
                    'media': media, 'mediana': mediana, 'moda': moda, 'varianza': varianza, 'desviacion': desviacion,
                    'q1': q1, 'q3': q3, 'iqr': iqr, 'freq_rel': freq_rel, 'freq_acum': freq_acum.tolist(), 'freq_rel_acum': freq_rel_acum.tolist()
                }
    ...
    with col2:
        if 'resultados_agrupados' in st.session_state:
            ...
            col_res1, col_res2 = st.columns(2)
            with col_res1:
                st.metric("Total de datos", res['n']); st.metric("Media", f"{res['media']:.4f}"); st.metric("Mediana", f"{res['mediana']:.4f}"); st.metric("Moda", f"{res['moda']:.4f}")
            with col_res2:
                st.metric("Varianza", f"{res['varianza']:.4f}"); st.metric("Desviación Estándar", f"{res['desviacion']:.4f}"); st.metric("Q1", f"{res['q1']:.4f}"); st.metric("Q3", f"{res['q3']:.4f}")
            st.subheader("📊 Tabla de Frecuencias")
            df_frec = pd.DataFrame({'Clase': res['clases'], 'Marca': [round(m, 2) for m in res['marcas_clase']], 'f': res['frecuencias'], 'fr(%)': [round(f, 2) for f in res['freq_rel']], 'F': res['freq_acum'], 'F(%)': [round(f, 2) for f in res['freq_rel_acum']]})
            st.dataframe(df_frec, use_container_width=True)
            st.subheader("📈 Visualizaciones")
            ...
            fig_pol_ojiva.add_trace(go.Scatter(x=res['marcas_clase'], y=res['frecuencias'], mode='lines+markers', name='Polígono', line=dict(color='#e74c3c', width=2)), row=1, col=1)
...
with tab3:
    ...
    with col1:
        ...
        if st.button("🔍 Calcular", key="calcular_cuali"):
            ...
            if categorias is None: st.error("❌ Error: Ingresa al menos 2 categorías válidas.")
            else:
                valores, conteos = np.unique(categorias, return_counts=True); n = len(categorias); porcentajes = conteos / n * 100
                max_freq = max(conteos); modas = valores[conteos == max_freq]
                st.session_state['resultados_cuali'] = {'nombres': valores.tolist(), 'frecuencias': conteos.tolist(), 'porcentajes': porcentajes.tolist(), 'n': n, 'modas': modas.tolist(), 'max_freq': max_freq}
                st.success(f"✅ Cálculo completado. Total de datos: {n}")
    with col2:
        if 'resultados_cuali' in st.session_state:
            ...
            st.subheader("📌 Resumen de Resultados")
            col_res1, col_res2 = st.columns(2)
            with col_res1: st.metric("Total de datos", res['n']); st.metric("Número de categorías", len(res['nombres']))
            with col_res2: modas_str = ", ".join(res['modas']); st.metric("Moda", f"{modas_str} (f: {res['max_freq']})")
            st.subheader("📊 Tabla de Frecuencias")
            df_cuali = pd.DataFrame({'Categoría': res['nombres'], 'Frecuencia': res['frecuencias'], 'Porcentaje': [round(p, 2) for p in res['porcentajes']]})
            st.dataframe(df_cuali, use_container_width=True)
            st.subheader("📈 Visualizaciones")
            col_graf1, col_graf2 = st.columns(2)
            with col_graf1:
                fig_bar = px.bar(df_cuali, x='Categoría', y='Frecuencia', color='Categoría', text=[f"{f} ({round(p, 1)}%)" for f, p in zip(res['frecuencias'], res['porcentajes'])], title='Gráfico de Barras')
            ...
            with col_graf2:
                fig_pie = px.pie(df_cuali, values='Frecuencia', names='Categoría', title='Gráfico Circular', color_discrete_sequence=px.colors.qualitative.Set3)
...
with tab4:
    ...
    with col1:
        ...
        if st.button("🔍 Calcular", key="calcular_cuali_frec"):
            ...
            if resultado is None: st.error("❌ Error: Ingresa al menos 2 categorías con frecuencias válidas.")
            else:
                ...
                st.session_state['resultados_cuali_frec'] = {'categorias': resultado['categorias'], 'frecuencias': resultado['frecuencias'], 'porcentajes': porcentajes, 'n': n, 'modas': modas, 'max_freq': max_freq}
                st.success(f"✅ Cálculo completado. Total de datos: {n}")
    with col2:
        if 'resultados_cuali_frec' in st.session_state:
            ...
            st.subheader("📌 Resumen de Resultados")
            col_res1, col_res2 = st.columns(2)
            with col_res1: st.metric("Total de datos", res['n']); st.metric("Número de categorías", len(res['categorias']))
            with col_res2: modas_str = ", ".join(res['modas']); st.metric("Moda", f"{modas_str} (f: {res['max_freq']})")
            st.subheader("📊 Tabla de Frecuencias")
            df_cuali_frec = pd.DataFrame({'Categoría': res['categorias'], 'Frecuencia': res['frecuencias'], 'Porcentaje': [round(p, 2) for p in res['porcentajes']]})
            st.dataframe(df_cuali_frec, use_container_width=True)
            st.subheader("📈 Visualizaciones")
            col_graf1, col_graf2 = st.columns(2)
            with col_graf1:
                fig_bar = px.bar(df_cuali_frec, x='Categoría', y='Frecuencia', color='Categoría', text=[f"{f} ({round(p, 1)}%)" for f, p in zip(res['frecuencias'], res['porcentajes'])], title='Gráfico de Barras')
                fig_bar.update_traces(textposition='outside'); fig_bar.update_layout(showlegend=False, height=400); st.plotly_chart(fig_bar, use_container_width=True)
            with col_graf2:
                fig_pie = px.pie(df_cuali_frec, values='Frecuencia', names='Categoría', title='Gráfico Circular', color_discrete_sequence=px.colors.qualitative.Set3)
                fig_pie.update_traces(textinfo='label+percent'); fig_pie.update_layout(height=400); st.plotly_chart(fig_pie, use_container_width=True)

# PESTAÑA 5: TABLA DE CONTINGENCIA (4 GRÁFICOS)
with tab5:
    col1, col2 = st.columns([1, 2])
    with col1:
        st.subheader("📊 Ingreso de Tabla de Contingencia")
        st.caption("Formato: fila1_col1, valor; fila1_col2, valor; ...")
        datos_contingencia = st.text_area("Datos de la tabla:", placeholder='Ejemplo: Hombre_Matemáticas, 15; Hombre_Ingeniería, 30; Mujer_Matemáticas, 15; Mujer_Ingeniería, 20', height=150)
        ...
        if st.button("🔍 Calcular", key="calcular_contingencia"):
            ...
            if len(filas_vec) < 2 or len(columnas_vec) < 2: st.error("❌ Error: Se necesitan al menos 2 filas y 2 columnas.")
            else:
                    ...
                    if len(elementos) == 2:
                        ...
                        try:
                            ...
                            if valor >= 0:
                                ...
                                if len(partes) == 2:
                                    fila = partes[0].strip(); columna = partes[1].strip()
                                    if fila in filas_vec and columna in columnas_vec:
                                        i = filas_vec.index(fila); j = columnas_vec.index(columna); matriz[i, j] = valor
                        except: continue
                if np.sum(matriz) == 0: st.error("❌ Error: No se encontraron datos válidos.")
                else:
                    st.session_state['resultados_contingencia'] = {'matriz': matriz, 'filas': filas_vec, 'columnas': columnas_vec}
...
with tab6:
    ...
    with col1:
        ...
        datos_x = st.text_area("Variable 1 (X) - Cuantitativa:", placeholder="Ejemplo: 12, 15, 18, 20, 22", height=80)
        datos_y = st.text_area("Variable 2 (Y) - Cuantitativa:", placeholder="Ejemplo: 25, 30, 35, 40, 45", height=80)
        if st.button("🔍 Calcular", key="calcular_biv"):
            resultado = procesar_datos_biv(datos_x, datos_y)
            if resultado is None: st.error("❌ Error: Ambas variables deben tener al menos 3 valores y la misma longitud.")
            else:
                x = resultado['x']; y = resultado['y']; n = len(x)
                media_x = np.mean(x); media_y = np.mean(y); var_x = np.var(x, ddof=1); var_y = np.var(y, ddof=1); sd_x = np.std(x, ddof=1); sd_y = np.std(y, ddof=1)
                cor_pearson, p_value = stats.pearsonr(x, y)
                slope, intercept, r_value, p_value_reg, std_err = stats.linregress(x, y)
                r2 = r_value**2; r2_adj = 1 - (1 - r2) * (n - 1) / (n - 2)
                x_pred = np.linspace(min(x) - 0.1*(max(x)-min(x)), max(x) + 0.1*(max(x)-min(x)), 100); y_pred = intercept + slope * x_pred
                y_pred_train = intercept + slope * x; residuos = y - y_pred_train
                st.session_state['resultados_biv'] = {'x': x, 'y': y, 'n': n, 'media_x': media_x, 'media_y': media_y, 'var_x': var_x, 'var_y': var_y, 'sd_x': sd_x, 'sd_y': sd_y, 'cor_pearson': cor_pearson, 'p_value': p_value, 'intercepto': intercept, 'pendiente': slope, 'r2': r2, 'r2_adj': r2_adj, 'x_pred': x_pred, 'y_pred': y_pred, 'residuos': residuos}
    ...
    with col2:
        if 'resultados_biv' in st.session_state:
            ...
            col_res1, col_res2 = st.columns(2)
            with col_res1:
                st.metric("Tamaño de muestra", res['n']); st.metric("Media X", f"{res['media_x']:.4f}"); st.metric("Media Y", f"{res['media_y']:.4f}"); st.metric("Correlación (r)", f"{res['cor_pearson']:.4f}")
            with col_res2:
                st.metric("Var X", f"{res['var_x']:.4f}"); st.metric("Var Y", f"{res['var_y']:.4f}"); st.metric("R²", f"{res['r2']:.4f}"); st.metric("R² ajustado", f"{res['r2_adj']:.4f}")
            st.subheader("📊 Interpretación")
            r_abs = abs(res['cor_pearson'])
            fuerza = "Muy fuerte" if r_abs >= 0.9 else "Fuerte" if r_abs >= 0.7 else "Moderada" if r_abs >= 0.5 else "Débil" if r_abs >= 0.3 else "Muy débil"
            direccion = "positiva" if res['cor_pearson'] > 0 else "negativa" if res['cor_pearson'] < 0 else "nula"
            col_int1, col_int2 = st.columns(2)
            with col_int1: st.info(f"**Correlación {fuerza} ({direccion})**")
            with col_int2:
                if res['p_value'] < 0.05: st.success("✅ Estadísticamente significativa (p < 0.05)")
                else: st.warning("❌ No significativa (p ≥ 0.05)")
            st.subheader("📊 Datos")
            df_biv = pd.DataFrame({'X': res['x'], 'Y': res['y']}); st.dataframe(df_biv, use_container_width=True)
            st.subheader("📈 Visualizaciones")
            graf_biv1, graf_biv2, graf_biv3 = st.tabs(["Dispersión + Regresión", "Correlación", "Residuos"])
            with graf_biv1:
                fig_scatter = go.Figure()
                fig_scatter.add_trace(go.Scatter(x=res['x'], y=res['y'], mode='markers', name='Datos', marker=dict(color='#3498db', size=10)))
                fig_scatter.add_trace(go.Scatter(x=res['x_pred'], y=res['y_pred'], mode='lines', name='Regresión', line=dict(color='#e74c3c', width=2)))
                fig_scatter.update_layout(title=f"Y = {res['intercepto']:.4f} + {res['pendiente']:.4f} * X", xaxis_title="Variable X", yaxis_title="Variable Y", height=450)
                st.plotly_chart(fig_scatter, use_container_width=True)
            with graf_biv2:
                matriz_cor = np.array([[1, res['cor_pearson']], [res['cor_pearson'], 1]])
                fig_cor = px.imshow(matriz_cor, x=['X', 'Y'], y=['X', 'Y'], text_auto=True, aspect="auto", title="Matriz de Correlación", color_continuous_scale='RdBu', zmin=-1, zmax=1)
                fig_cor.update_layout(height=450); st.plotly_chart(fig_cor, use_container_width=True)
            with graf_biv3:
                fig_residuos = go.Figure(); fig_residuos.add_trace(go.Scatter(x=res['x'], y=res['residuos'], mode='markers', name='Residuos', marker=dict(color='#e74c3c', size=10)))
                fig_residuos.add_hline(y=0, line_dash="dash", line_color="black")
                fig_residuos.update_layout(title="Gráfico de Residuos", xaxis_title="Variable X", yaxis_title="Residuos", height=450)
                st.plotly_chart(fig_residuos, use_container_width=True)
...
with tab7:
    ...
    ### Media (Promedio)
    - Es el promedio aritmético de todos los datos; Representa el centro de masa de la distribución; **Sensible a outliers**.
    ### Mediana
    - Valor que divide los datos en dos partes iguales (50% cada una); **Robusta a outliers**; Ideal para distribuciones asimétricas.
    ### Moda
    - Valor que aparece con mayor frecuencia; Puede haber más de una moda; Útil para datos categóricos.
    ## 📊 Medidas de Dispersión
    ### Varianza y Desviación Estándar
    - Miden la dispersión. La desviación estándar tiene la misma unidad que los datos.
    ### Rango
    - Diferencia entre el máximo y el mínimo; **Muy sensible a outliers**.
    ### Rango Intercuartil (IQR)
    - Q3 - Q1 (diferencia entre el percentil 75 y 25); **Robusto a outliers**.
    ## 🔗 Análisis Bivariado
    ### Correlación de Pearson (r)
    - 0.90 - 1.00: Muy fuerte; 0.70 - 0.89: Fuerte; 0.50 - 0.69: Moderada; 0.30 - 0.49: Débil; 0.00 - 0.29: Muy débil.
    ### Significancia (p-value)
    - **p < 0.05**: Resultado estadísticamente significativo.
    ### Chi-cuadrado (χ²)
    - Mide la asociación entre variables categóricas.
    """)
