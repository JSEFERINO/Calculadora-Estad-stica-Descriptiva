...
# ============================================================
# CORRECCIÓN: INICIALIZAR SESSION STATE
# ============================================================
...
# ============================================================
# FUNCIONES AUXILIARES PARA ANÁLISIS DE ARCHIVOS
# ============================================================
...
def identificar_distribucion(datos):
    ...
    if not distribuciones_posibles:
        if abs(skewness) > 1: distribuciones_posibles.append("Asimétrica (sesgo significativo)")
        elif abs(kurtosis) > 3: distribuciones_posibles.append("Con colas pesadas (leptocúrtica)")
        else: distribuciones_posibles.append("Sin distribución clara")
...
# ============================================================
# INTERFAZ DE USUARIO - BARRA LATERAL (Carga de archivos)
# ============================================================
...
def mostrar_analisis_descriptivo(df):
        ...
        if var1 != var2 and st.button("📊 Analizar Relación Bivariada", key="btn_bivariado"):
            ...
            if resultados['tipo'] == 'Cualitativa-Cualitativa':
                ...
                if 'pendiente' in resultados['estadisticos']:
                    ...
                    fig_scatter.add_trace(go.Scatter(x=x_range, y=y_reg, mode='lines', name='Regresión', line=dict(color='#e74c3c', width=2)))
                ...
                fig_box = px.box(df[[var_cat, var_cuant]].dropna(), x=var_cat, y=var_cuant, color=var_cat, title="Boxplots por Categoría", points="all")
    ...
    if columnas_numericas:
        ...
        columna_seleccionada = st.selectbox("Selecciona una variable numérica:", columnas_numericas)
        ...
        if len(datos) > 0:
            ...
            stats_df = pd.DataFrame({
                'Estadística': ['N', 'Media', 'Mediana', 'Moda', 'Varianza', 'Desviación', 'Mínimo', 'Máximo', 'Rango', 'Q1', 'Q3', 'IQR'],
                'Valor': [
                    len(datos), f"{np.mean(datos):.4f}", f"{np.median(datos):.4f}", moda_str, f"{np.var(datos, ddof=1):.4f}",
                    f"{np.std(datos, ddof=1):.4f}", f"{np.min(datos):.4f}", f"{np.max(datos):.4f}",
                    f"{np.max(datos) - np.min(datos):.4f}", f"{np.percentile(datos, 25):.4f}",
                    f"{np.percentile(datos, 75):.4f}", f"{np.percentile(datos, 75) - np.percentile(datos, 25):.4f}"
                ]
            })
    ...
    # ANÁLISIS UNIVARIADO (CATEGÓRICO) - CORREGIDO CON GRÁFICOS
    if columnas_categoricas:
        st.subheader("📊 Análisis de Variables Categóricas")
        
        # Si no hay una columna guardada en sesión, seleccionar la primera
...
# Verificar si hay un DataFrame cargado en la sesión
if st.session_state.df_cargado is not None:
    df_cargado = st.session_state.df_cargado
    mostrar_analisis_descriptivo(df_cargado)
    st.markdown("---")
else:
    st.info("📂 Carga un archivo CSV o URL desde la barra lateral para comenzar el análisis.")

# ============================================================
# PESTAÑAS COMPLETAS (ANÁLISIS MANUAL FUNCIONAL)
# ============================================================
...
# ============================================================
# PESTAÑA 1: DATOS EN LISTA (CORREGIDO VISUALMENTE)
# ============================================================
...
with tab1:
    ...
    with col1:
        ...
        if st.button("🔍 Calcular", key="calcular_lista"):
            ...
            if datos is None:
                ...
                # Cálculo de clases corregido para visualización
    ...
    with col2:
        if 'resultados_lista' in st.session_state:
            ...
            # Histograma con polígono corregido (ancho de barra calculado)
            ...
            fig_hist.update_layout(title="Histograma y Polígono de Frecuencias", height=350, barmode='overlay', xaxis_title="Clases", yaxis_title="Frecuencia")
            st.plotly_chart(fig_hist, use_container_width=True)

            # Ojiva CORREGIDA (usando límites superiores)
            fig_ojiva = go.Figure()
            ...
            fig_ojiva.update_layout(title="Ojiva (Frecuencias Acumuladas)", height=350, xaxis_title="Límite Superior de Clase", yaxis_title="Frecuencia Acumulada")
...
with tab6:
    ...
    with col1:
        ...
        if st.button("🔍 Calcular", key="calcular_biv"):
            ...
            if resultado is None:
                ...
                media_x = np.mean(x); media_y = np.mean(y); var_x = np.var(x, ddof=1); var_y = np.var(y, ddof=1)
                sd_x = np.std(x, ddof=1); sd_y = np.std(y, ddof=1)
                ...
                st.session_state['resultados_biv'] = {
                    'x': x, 'y': y, 'n': n, 'media_x': media_x, 'media_y': media_y,
                    'var_x': var_x, 'var_y': var_y, 'sd_x': sd_x, 'sd_y': sd_y,
                    'cor_pearson': cor_pearson, 'p_value': p_value, 'intercepto': intercept,
                    'pendiente': slope, 'r2': r2, 'r2_adj': r2_adj, 'x_pred': x_pred,
                    'y_pred': y_pred, 'residuos': residuos
                }
    ...
    with col2:
        if 'resultados_biv' in st.session_state:
            ...
            col_res1, col_res2 = st.columns(2)
            with col_res1:
                st.metric("Tamaño de muestra", res['n']); st.metric("Media X", f"{res['media_x']:.4f}")
                st.metric("Media Y", f"{res['media_y']:.4f}"); st.metric("Correlación (r)", f"{res['cor_pearson']:.4f}")
            with col_res2:
                st.metric("Var X", f"{res['var_x']:.4f}"); st.metric("Var Y", f"{res['var_y']:.4f}")
                st.metric("R²", f"{res['r2']:.4f}"); st.metric("R² ajustado", f"{res['r2_adj']:.4f}")
            ...
            with graf_biv1:
                ...
                fig_scatter.update_layout(title=f"Y = {res['intercepto']:.4f} + {res['pendiente']:.4f} * X", xaxis_title="Variable X", yaxis_title="Variable Y", height=450); st.plotly_chart(fig_scatter, use_container_width=True)
            with graf_biv2:
            ...
            with graf_biv3:
                ...
                fig_residuos.update_layout(title="Gráfico de Residuos", xaxis_title="Variable X", yaxis_title="Residuos", height=450); st.plotly_chart(fig_residuos, use_container_width=True)
